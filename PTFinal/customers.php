...
<head>
 <meta charset="utf-8">
  ...
  <title>My Bird Shop Ordering System : Customers</title>
   <!-- Bootstrap -->
...
<body>
 ...
 <div class="container-fluid">
    ...
    <form action="customers.php" method="post" class="form-horizontal">
     ...
     <div class="col-sm-9">
         <input name="contact" type="text" class="form-control"  id="contact" placeholder="Phone number" value="<?php if(isset($_GET['edit'])) echo $editrow['FLD_CUSTOMER_CONTACT']; ?>" required> 
     </div>
     </div>
     ...
     <div class="col-sm-9">
     <textarea name="address" class="form-control" id="address" rows="4" cols="50" placeholder="Enter address"><?php if(isset($_GET['edit'])) echo $editrow['FLD_CUSTOMER_ADDRESS']; ?></textarea>
     </div>
     </div>
...
<div class="row">
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
        ...
        $stmt = $conn->prepare("select * from tbl_customers_a175432_pt2 LIMIT $start_from, $per_page");
        $stmt->execute();
      ...
      <tr>
        ...
        <td><?php echo $readrow['FLD_CUSTOMER_CONTACT']; ?></td>
        ...
        <td style="<?php echo $hide ?>">
          ...
          <a href="customers.php?delete=<?php echo $readrow['FLD_CUSTOMER_ID']; ?>" class="btn btn-danger btn-xs" role="button" onclick="return confirm('Are you sure to delete?');" >Delete</a>
        </td>
      </tr>
      <?php
      }
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_customers_a175432_pt2");
            $stmt->execute();
            $result = $stmt->fetchAll();
