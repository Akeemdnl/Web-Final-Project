...
<body>
  ...
  <div class="container-fluid">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
            ...
            <option value="">Please select</option>
             <?php
              try {
                $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                  $stmt = $conn->prepare("SELECT * FROM tbl_customers_a175432_final");
                $stmt->execute();
                $result = $stmt->fetchAll();
              }
              catch(PDOException $e){
     ...
     <input type="hidden" name="oldcid" value="<?php echo $editrow['fld_order_num']; ?>">
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a175432_final, tbl_staffs_a175432_final, tbl_customers_a175432_final WHERE ";
        $sql = $sql."tbl_orders_a175432_final.FLD_STAFF_ID = tbl_staffs_a175432_final.FLD_STAFF_ID and ";
        $sql = $sql."tbl_orders_a175432_final.FLD_CUSTOMER_ID = tbl_customers_a175432_final.FLD_CUSTOMER_ID LIMIT $start_from, $per_page";
      ...
      <tr>
        ...
        <td>
          ...
          <a style="<?php echo $hide ?>" href="orders.php?edit=<?php echo $orderrow['FLD_ORDER_ID']; ?>" class="btn btn-success btn-xs"  role="button">Edit</a>
          <a style="<?php echo $hide ?>" href="orders.php?delete=<?php echo $orderrow['FLD_ORDER_ID']; ?>" class="btn btn-danger btn-xs"  role="button" onclick="return confirm('Are you sure to delete?');">Delete</a>
        </td>
      </tr>
      <?php
      }
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a175432_final");
            $stmt->execute();
            $result = $stmt->fetchAll();
